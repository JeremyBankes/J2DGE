...
public class KeyInput {
	...
	private static final int DOUBLE_TAP_THRESHOLD = 250;

	private HashMap<Integer, Long> doubleTaps;
	private HashSet<KeyEventCallback> keyCallbacks;
	private HashSet<Integer> pressed;

	public KeyInput(Component component) {
		doubleTaps = new HashMap<>();
		keyCallbacks = new HashSet<>();
		pressed = new HashSet<>();
	...
	public boolean isPressed(int asciiCode) {
		return pressed.contains(asciiCode);
	}

	public boolean isAnyPressed(int... asciiCodes) {
	...
	public boolean isAllPressed(int... asciiCodes) {
		for (int asciiCode : asciiCodes) {
			if (!pressed.contains(asciiCode)) {
				return false;
			}
	...
	public void removeKeyEventCallback(KeyEventCallback callback) {
		keyCallbacks.remove(callback);
	}

	public static enum KeyAction {
		PRESS, RELEASE, DOUBLE_TAP, TYPE
	}

	public static interface KeyEventCallback {

		public abstract void invoke(KeyInputEvent event);
